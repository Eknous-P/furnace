...
void DivPlatformSID2::getPaired(int ch, std::vector<DivChannelPair>& ret) {
  if (chan[ch].ring) {
    if (ch==0) {
      ret.push_back(DivChannelPair(_("ring"),PAIR_ARROW_NONE,2));
    } else {
      ret.push_back(DivChannelPair(_("ring"),PAIR_ARROW_NONE,(ch-1)%3));
    }
  }
  if (chan[ch].sync) {
    if (ch==0) {
      ret.push_back(DivChannelPair(_("sync"),PAIR_ARROW_NONE,2));
    } else {
      ret.push_back(DivChannelPair(_("sync"),PAIR_ARROW_NONE,(ch-1)%3));
    }
  }
}
